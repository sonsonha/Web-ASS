document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchUserProfile() {
        try {
            const response = await fetch('/views/user/test_api/fetch_user_profile.php', {
                method: 'POST',
    ...
    async function fetchCartItems() {
        try {
            const response = await fetch('/views/user/test_api/fetch_cart.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ user_id: userId }),
            });
    ...
    function renderCartItems(items) {
        ...
        items.forEach((item) => {
            ...
            cartItem.querySelector('.game-image').addEventListener('click', (e) => {
                ...
                window.location.href = `/views/games/detail.php?id=${gameId}`;
            });
    ...
    buyNowButton.addEventListener('click', () => {
        ...
        if (userCoins < total) {
            ...
            fetch('/views/user/test_api/confirm_purchase.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    user_id: userId,
    ...
    addCoinsButton.addEventListener('click', () => {
        window.location.href = '/views/user/add_coins.php'; // Navigate to Add Coins page
    });
    ...
    async function removeItemFromCart(userId, gameId) {
        try {
            const response = await fetch('/views/user/test_api/remove_from_cart.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
