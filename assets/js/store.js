    ...
    const renderCards = () => {
        ...
        for (let i = 0; i < cardsPerPage; i++) {
            ...
            cardElement.querySelector('.category-card').addEventListener('click', () => {
                window.location.href = `/views/store/category.php?category=${category.slug}`;
            });
        }
    };
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const response = await fetch("/views/store/test_api/fetch_carousel_games.php");
    ...
    const renderCarousel = () => {
        ...
        topRatedGames.forEach((game, index) => {
            ...
            // Click event for the slide (navigate to game game)
            // slide.addEventListener("click", () => {
            //     window.location.href = `/views/games/game.php?id=${game.id}`;
            // });
            slide.addEventListener("click", () => {
                const gameId = game.id;
            
                // Thay đổi URL mà không làm mới trang
                history.pushState({ gameId: gameId }, "", "/game"); // URL sẽ chỉ là "/game"
            
                // Logic tải game (vẫn giữ nguyên)
                window.location.href = `/views/games/game.php?id=${gameId}`;
            });
        ...
        document.querySelectorAll(".play-free").forEach((button) => {
            button.addEventListener("click", (e) => {
                e.stopPropagation(); // Ngừng sự kiện click slide
                const gameId = button.getAttribute("data-id");
        
                // Thay đổi URL trong thanh địa chỉ mà không tải lại trang
                history.pushState({ gameId: gameId }, "", "/game");
        
                // Tải nội dung game và hiển thị trên trang mà không làm mới
                fetch(`/views/games/game.php?id=${gameId}`)
                    .then(response => response.text()) // Lấy nội dung HTML của game
                    .then(html => {
                        // Chèn nội dung game vào phần tử thích hợp trên trang
    ...
    async function addToCart(gameId) {
        try {
            ...
            const response = await fetch("/views/user/test_api/add_to_cart.php", {
                method: "POST",
    ...
    async function buyGame(gameId) {
        try {
            const userId = 1; // Example user ID, replace with actual logic
            const response = await fetch("/views/user/test_api/buy_game.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ user_id: userId, game_id: gameId }),
            });
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const response = await fetch("/views/store/test_api/fetch_trending_games.php");
