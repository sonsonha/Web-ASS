document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const userData = await fetchData('/views/user/test_api/fetch_user_profile.php', { user_id: userId });
        populateUserProfile(userData);

        if (userData['game-own'] && userData['game-own'].length > 0) {
            const gamesData = await fetchData('/views/user/test_api/fetch_games.php', { game_ids: userData['game-own'] });
    ...
    EditProfile.addEventListener('click', () => {
        window.location.href = '/views/user/Edit_profile.php';
    });
});
...
function populateGamesOwned(games) {
    ...
    games.forEach((game) => {
        ...
        gameCard.innerHTML = `
        <a href="/views/games/detail.php?id=${game.id}">
