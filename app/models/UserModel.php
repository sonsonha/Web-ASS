...
class UserModel {
    ...
    public function getShopingCart($id) {
        ...
        $cartQuery = "
            SELECT g.id AS game_id, g.game_name, g.price, g.avt
            FROM don_hang dh
            LEFT JOIN chi_tiet_don_hang ctdh ON dh.id = ctdh.order_id
            LEFT JOIN game g ON ctdh.game_id = g.id
            WHERE dh.user_id = :user_id AND dh.status = 'Paid'
        ";
    ...
    public function updateProfile($user_id, $fullName, $phone, $url_link, $birth) {
        $query = "
            UPDATE tai_khoan
            SET full_name = :fullName, phone_number = :phone, avatar = :url_link, date_of_birth = :birth
            WHERE id = :user_id
        ";
